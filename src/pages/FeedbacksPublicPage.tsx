...
const IconMoodSad = (props: React.ComponentProps<'svg'>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0" /><path d="M9 10l.01 0" /><path d="M15 10l.01 0" /><path d="M9.5 15.25a3.5 3.5 0 0 1 5 0" /></svg>
);
...
const FeedbacksPublicPage = () => {
  ...
  useEffect(() => {
    const fetchFeedbacks = async () => {
      ...
      if (fetchError) { console.error('Erro:', fetchError); setError('Não foi possível carregar os feedbacks.'); } else { setFeedbacks(data); }
      setLoading(false);
    };
    fetchFeedbacks();
  }, []);
        ...
        <div className={classes.feedbackGrid}>
          {feedbacks.map((feedback) => (
            <Paper withBorder p="xl" radius="md" key={feedback.id} className={classes.card}>
              <Group><Avatar src={feedback.user_avatar_url} alt={feedback.user_display_name} radius="xl" size="lg" /><div><Text fz="lg" fw={500}>{feedback.user_display_name}</Text><StarRating rating={feedback.rating} /></div></Group>
              <Text pl={64} pt="sm" fz="sm">"{feedback.comment}"</Text>
            </Paper>
          ))}
        </div>
      )}
